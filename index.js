...
async function startGojoMdNx() {
  const GojoMdNx = NexusNwIncConnect({
    ...
    browser: ["Gojo SatoruNexus", "Safari", "1.0.0"],
    auth: state,
  });
  ...
  GojoMdNx.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    try {
      ...
      if (!GojoMdNx.public && !mek.key.fromMe && chatUpdate.type === "notify")
        return;
...
⭐✑ I'm Not Sure If It Was A Goodbye Charm, But It Was Fun While It Lasted 😌✨`,
  ...
  GojoMdNx.send5ButImg = async (
    ...
    GojoMdNx.relayMessage(jid, template.message, {
  ...
  GojoMdNx.sendImage = async (
    ...
    let buffer = Buffer.isBuffer(path)
      ...
      : Buffer.alloc(0);
    return await GojoMdNx.sendMessage(
      jid,
